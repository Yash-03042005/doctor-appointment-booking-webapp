  ...
  const getAvailableSlots = async()=>{
    ...
    for(let i = 0;i<7;i++){
      ...
      while(currentDate < endTime){
        ...
        //increment the time slots by 30 min
        ...
        <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
          {
            docSlots.length && docSlots[slotIndex].map((item,index)=>(
              ...
              <p onClick={()=>setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? `bg-primary text-white`:`text-gray-300 border border-gray-300`} `} key={index} >
